...
class ResultRemoteMediatorTest {
    ...
    fun `REFRESH clears cache, upserts first page, and saves sync state on success`() = runTest {
        // given
        val firstPage = ResultPage(results = MockData.Api.Results.results, currentPage = 1, totalPages = 2)
        ...
        val mediator = ResultRemoteMediator(
            ...
            resultsApi = mockk<ResultsApi> {
                coEvery { fetchResults(session.accessToken, page = 1, pageSize = config.pageSize) } returns firstPage
            },
        ...
        coVerify(exactly = 1) {
            syncStateDao.upsert(
                ResultSyncStateEntity(lastLoadedPage = 1, totalPages = 2, endOfPaginationReached = false),
            )
        }
    }
    ...
    fun `REFRESH reports endOfPaginationReached when only one page exists`() = runTest {
        ...
        val mediator = ResultRemoteMediator(
            ...
            resultsApi = mockk<ResultsApi> {
                coEvery { fetchResults(session.accessToken, page = 1, pageSize = config.pageSize) } returns onlyPage
            },
            authRepository = mockk<AuthRepository> {
                coEvery { validSession() } returns Result.success(session)
            },
    ...
    fun `APPEND fetches next page based on saved sync state`() = runTest {
        // given
        val secondPage = ResultPage(results = MockData.Api.Results.results, currentPage = 2, totalPages = 2)
        val resultDao = mockk<ResultDao>(relaxUnitFun = true)
        val syncStateDao = mockk<ResultSyncStateDao>(relaxUnitFun = true) {
            coEvery { get() } returns
                ResultSyncStateEntity(lastLoadedPage = 1, totalPages = 2, endOfPaginationReached = false)
        ...
        val mediator = ResultRemoteMediator(
            ...
            resultSyncStateDao = syncStateDao,
            resultsApi = mockk<ResultsApi> {
                coEvery { fetchResults(session.accessToken, page = 2, pageSize = config.pageSize) } returns secondPage
            },
            authRepository = mockk<AuthRepository> {
                coEvery { validSession() } returns Result.success(session)
            },
        ...
        coVerify(exactly = 1) {
            syncStateDao.upsert(
                ResultSyncStateEntity(lastLoadedPage = 2, totalPages = 2, endOfPaginationReached = true),
            )
        }
    }

    @Test
    fun `APPEND ends pagination immediately when saved sync state already reached the end`() = runTest {
        // given
        val resultsApi = mockk<ResultsApi>()
        val mediator = ResultRemoteMediator(
            resultDao = mockk(),
        ...
        assertThat(result).isInstanceOf<RemoteMediator.MediatorResult.Success>()
        assertThat((result as RemoteMediator.MediatorResult.Success).endOfPaginationReached).isTrue()
        coVerify(exactly = 0) { resultsApi.fetchResults(any(), any(), any()) }
    }
    ...
    fun `REFRESH returns Error when the API throws`() = runTest {
        ...
        val mediator = ResultRemoteMediator(
            ...
            resultSyncStateDao = mockk(),
            resultsApi = mockk {
                coEvery { fetchResults(session.accessToken, page = 1, pageSize = config.pageSize) } throws apiFailure
            },
            authRepository = mockk {
                coEvery { validSession() } returns Result.success(session)
            },
