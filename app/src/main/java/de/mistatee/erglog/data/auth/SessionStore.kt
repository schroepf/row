...
private val KEY_ACCESS_TOKEN_EXPIRY_EPOCH_SECONDS = longPreferencesKey("access_token_expiry_epoch_seconds")

internal val Context.sessionDataStore: DataStore<Preferences> by
    preferencesDataStore(name = PREFS_FILE_NAME)
...
class EncryptedSessionStore(context: Context) : SessionStore {
    ...
    override suspend fun getSession(): Session? {
        ...
        return try {
            Session(
                ...
                accessTokenExpiry = Instant.ofEpochSecond(stored.expiryEpochSeconds),
            )
        } catch (_: Exception) {
    ...
    private fun readStoredSession(preferences: Preferences): StoredSession? {
        ...
        val expiryEpochSeconds = preferences[KEY_ACCESS_TOKEN_EXPIRY_EPOCH_SECONDS]

        return accessTokenCipher?.let { accessCipher ->
            refreshTokenCipher?.let { refreshCipher ->
                expiryEpochSeconds?.let { expiry ->
                    StoredSession(
    ...
    override suspend fun saveSession(session: Session) {
        ...
        dataStore.edit { preferences ->
            ...
            preferences[KEY_ACCESS_TOKEN_EXPIRY_EPOCH_SECONDS] = session.accessTokenExpiry.epochSecond
        }
    }
